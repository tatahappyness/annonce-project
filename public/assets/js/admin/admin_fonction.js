  ...
  function setPopular(id, pop){			
    ...
    xhttp.send();
		...
		function show_modal_add_fonction(){
			
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (xhttp.readyState == 4 && xhttp.status == 200) {
                   document.getElementById("load_model_fonction_add").innerHTML = xhttp.responseText;
               }
            };

            xhttp.open("GET", "/admin/fonction/newFonction", true);
            xhttp.send();
        }

		function show_modal_sup_fonction($id){
			
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (xhttp.readyState == 4 && xhttp.status == 200) {
                   document.getElementById("load_model_fonction_supprimer").innerHTML = xhttp.responseText;
               }
            };

            xhttp.open("GET", "/admin/fonction/fonctionDelete/"+$id, true);
            xhttp.send();
   }